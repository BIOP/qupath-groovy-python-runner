...
def model = "dsb_heavy"
def model_folder = new File ("D:\\StatDist Models")

def stardist_parameters = [ "model":model, "model_folder":model_folder.getAbsolutePath() ]
...
def stardist_env_folder = 'D:\\environments\\stardist'

// The location of the script that will process the images and return RoiSets
def stardist_python_file = 'D:\\qupath-groovy-python-runner\\Examples\\StarDist\\stardist_to_ij_rois.py'

// This line make sure that everything is ready to go
def pRStarDist = new PythonRoiRunner( stardist_env_folder, stardist_python_file)
                                    .setDownsample(2)
                                    .setParameters( stardist_parameters )
                                    
// Process the currently selected object                  
pRStarDist.processRegion(getSelectedObject(), preprocess )
